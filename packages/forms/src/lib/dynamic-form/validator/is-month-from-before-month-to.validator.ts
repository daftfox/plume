...
export const validateIsMonthFromBeforeMonthTo = (control: AbstractControl) => {
  if (
    ...
    (control === control.parent.get('monthFrom') && control.parent.get('monthTo').value === null) || // monthTo value is null
    (control === control.parent.get('monthTo') && control.parent.get('monthFrom').value === null) // monthFrom value is null
  )
    return null;
