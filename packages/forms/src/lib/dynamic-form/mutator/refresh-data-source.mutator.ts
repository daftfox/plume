...
export const refreshDataSource: MutatorFn = <T = DynamicFormElementValueType>(
  originKey: string,
  targetKey: string,
  ...
  if (
    ...
    throw new Error(`Linked element ${targetKey} does not extend AbstractReactiveFormQuestionComponent or AbstractReactiveFormElementComponent.`);
  }

  (linkedElement as AbstractReactiveFormElementComponent<unknown>).refresh( new Map([[originKey, value]]) );
};
