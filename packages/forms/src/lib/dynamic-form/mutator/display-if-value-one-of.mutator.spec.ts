...
import { displayIfValueOneOf } from './display-if-value-one-of.mutator';
...
describe('displayIfValueOneOf', () => {
  const mockValue = 'mock';

  it('should display the control if control value is present in array of asserted values', () => {
    const serviceSpy = jest.spyOn(
      mockDynamicFormService,
      'setElementVisibility',
    );
    displayIfValueOneOf([mockValue])(
      'mockOrigin',
      mockFormControlKey,
      mockDynamicFormService,
      mockValue,
  ...
  it('should hide the control if control value is not present in array of asserted values', () => {
    const serviceSpy = jest.spyOn(
      mockDynamicFormService,
      'setElementVisibility',
    );
