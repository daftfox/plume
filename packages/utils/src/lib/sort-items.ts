...
export const sortItems = <T>( items: T[], active: keyof T, direction: SortDirection ): T[] => {
  if ( !active || direction === '' ) {
    return items;
  }
  ...
  return [...items].sort((a, b) => {
    ...
    if (isNullish(valueA) && !!valueB) {
      ...
      return naturalSortCollator.compare( valueA as string, valueB as string ) * (isAsc ? 1 : -1);
