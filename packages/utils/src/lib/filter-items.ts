...
export const filterItems = <T>( items: T[], filters: any ): T[] => {
  ...
  for (const i in filters) {
    ...
    if (filterValue !== '' && !isNullish(filterValue) && itemHasProperty(filteredData[0], i)) {
      if (typeof filterValue === 'string') {
        // Filter property is of type string. Verify item's property value is a substring of it
        filteredData = filteredData.filter((item) => (item[i as keyof T] as string).toLowerCase().indexOf(filterValue.toLowerCase()) !== -1);
        ...
        filteredData = filteredData.filter((item) => item[i as keyof T] === filterValue);
        ...
        filteredData = filteredData.filter((item) => filterValue.includes(item[i as keyof T]));
      }
    }
  }
  return filteredData;
...
const itemHasProperty = <T>( item: T, property: string ): boolean => Object.prototype.hasOwnProperty.call(item, property);
