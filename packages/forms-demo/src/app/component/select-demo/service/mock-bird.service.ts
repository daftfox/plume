...
export class MockBirdService {
  ...
  getByRegion(region: string): Observable<MockObservation[]> {
    ...
    return this.http
      .get<MockObservation[]>(
        `https://api.ebird.org/v2/data/obs/${region}/recent/notable?detail=full`,
        { headers },
      )
      .pipe(
        map((response) =>
          response.reduce(
            (observations: MockObservation[], item: MockObservation) => {
              if (
                !observations.find(({ sciName }) => sciName === item.sciName)
              ) {
  ...
  ): Observable<MockObservation> {
    return this.getByRegion(region).pipe(
      map((response: MockObservation[]) =>
        response.find(({ obsId }) => obsId === observationId),
      ),
